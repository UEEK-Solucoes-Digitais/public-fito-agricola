...
export async function GET(request: NextRequest, { params }: { params: { type: number } }): Promise<Response> {
    try {
        let filter = ''
        let page = ''
        ...
        const url = `/api/interference-factors/list/${params.type}?${filter}&${page}`

...
export const DiseasesTable: FC<ReportTableProps> = ({ currentQuery, setUrl }) => {
    const headers = [
        '#',
        'Propriedade',
        'Ano Agrícola',
    ...
    const style = `0.4fr 1.2fr repeat(4, 0.6fr)  1fr 0.6fr 1fr 0.8fr 0.8fr repeat(3, 1fr) 0.01fr`
    ...
    const { data, isLoading } = useSWR(`/api/reports/list/${admin.id}/diseases${currentQuery}${pageParam}`, getFetch)
    ...
    useEffect(() => {
        ...
        const url = `/api/reports/list/${admin.id}/diseases${currentQuery}${pageParam}`
        setUrl(url)
        mutate(url)
    }, [currentQuery])
            ...
            {data && data.reports ? (
                data.reports.map((item: any, index: number) => (
                    <TableRow
                        ...
                        <div data-type='content'>
                            <p title={item.disease?.name}>{item.disease?.name ?? '--'}</p>
                        </div>
                        ...
                        <div data-type='content'>
                            <p title={item.disease?.observations}>{item.disease?.observations ?? '--'}</p>
                        </div>

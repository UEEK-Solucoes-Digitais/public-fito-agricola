...
const DiseaseFilter: FC<FilterProps> = ({ currentQuery, setCurrentQuery }) => {
    ...
    const { data, isLoading } = useSWR(`/api/reports/get-filters-options/${admin.id}?with=diseases`, getFetch)
    ...
    return (
        <>
            ...
            <div className={styles.filterGroup}>
                <GeralInput name='diseases_id' label='Doenças' type='select' onChange={handleInputArrayChange}>
                    <option value='0'>Selecione</option>
                    ...
                    {data &&
                        (filterOptions.culture_id !== '0'
                            ? data.cultures
                                  .filter((item: any) => item.id == parseFloat(filterOptions.culture_id.toString()))
                                  .map((item: any) =>
                                      item.diseases.map((defensive: any) => (
                                          <option key={defensive.id} value={defensive.id}>
                                              {defensive.name}
                                          </option>
                                      )),
                                  )
                            : data.diseases.map((pest: Crop) => (
                ...
                <div className={styles.filterOptions}>
                    {data &&
                        data.diseases.map(
                            (disease: any) =>
                                filterOptions.diseases_id.includes(disease.id.toString()) && (
                                    <div key={disease.id} className={styles.filterOptionItem}>
                                        {disease.name}

                                        <button onClick={() => removeItem('diseases_id', disease.id.toString())}>
                                            <IconifyIcon icon='ph:x' />
                                        </button>
                                    </div>
                                ),

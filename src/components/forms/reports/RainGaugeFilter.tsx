...
const RainGaugeFilter: FC<FilterProps> = ({ currentQuery, setCurrentQuery }) => {
    ...
    const { data, isLoading } = useSWR(`/api/reports/get-filters-options/${admin.id}`)
    ...
    useEffect(() => {
        if (currentQuery) {
            const query = currentQuery.toString().replace('?', '')
                        ...
                        data.properties.map(
                            (property: Property) =>
                                filterOptions.properties_id.includes(property.id.toString()) && (
                                    <div key={`baloon-property-${property.id}`} className={styles.filterOptionItem}>
                                        {property.name}
                        ...
                        data.crops.map(
                            (crop: Crop) =>
                                filterOptions.crops_id.includes(crop.id.toString()) && (
                                    <div key={`baloon-crop-${crop.id}`} className={styles.filterOptionItem}>
                                        {crop.name}
                        ...
                        data.harvests.map(
                            (harvest: any) =>
                                filterOptions.harvests_id.includes(harvest.id.toString()) && (
                                    <div key={`baloon-harvest-${harvest.id}`} className={styles.filterOptionItem}>
                                        {harvest.name}
...
export default RainGaugeFilter

...
const WeedFilter: FC<FilterProps> = ({ currentQuery, setCurrentQuery }) => {
    ...
    const { data, isLoading } = useSWR(`/api/reports/get-filters-options/${admin.id}?with=weeds`, getFetch)
    ...
    return (
        <>
            ...
            <div className={styles.filterGroup}>
                <GeralInput name='weeds_id' label='Daninhas' type='select' onChange={handleInputArrayChange}>
                    <option value='0'>Selecione</option>

                    {data &&
                        data.weeds.map((weed: any) => (
                ...
                <div className={styles.filterOptions}>
                    {data &&
                        data.weeds.map(
                            (weed: any) =>
                                filterOptions.weeds_id.includes(weed.id.toString()) && (
                                    <div key={weed.id} className={styles.filterOptionItem}>
                                        {weed.name}

                                        <button onClick={() => removeItem('weeds_id', weed.id.toString())}>
                                            <IconifyIcon icon='ph:x' />
                                        </button>
                                    </div>
                                ),
                    ...
                    {[1, 2, 3].map(
                        (risk: number, index) =>
                            filterOptions.risk.includes(risk.toString()) && (
                                <div key={index} className={styles.filterOptionItem}>
